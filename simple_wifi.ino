...
void loop() {
  ...
  ccnt = ESP.getCycleCount();  // 2^32 / 80e6 = 53.7 seconds to rollover, esp clock is 80 mhz, so there are 80e6 cycles per second, and the count is stored in 32 bits. 
  ...
  if ((ccnt - rgbcycles) > 250000){  // this flashes the built-in led about once per second. one half second is about 0.5 / 160 / 80e6 = 250000
    ...
    // when getCycleCount rolls over, ccnt - rgbcycles will be negative, but I think the unsigned math results in a large unsigned value which enables this loop to handle the rollover with 
    // an error less than 3.2 msec
